...
class OrderAggregator:
    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        current_items_quantity = max_items_quantity
        customer_items = []
        all_items = self.order_items
        for item in all_items:
            if item.customer == customer:
                if (current_volume - item.total_volume) >= 0 and (current_items_quantity - item.quantity) >= 0:
                    current_volume -= item.total_volume
                    current_items_quantity -= item.quantity
                    customer_items.append(item)
        for items in customer_items:
            self.order_items.remove(items)
