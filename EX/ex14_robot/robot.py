...
def the_true_follower(robot: FollowerBot):
    ...
    while 0 in robot.get_line_sensors():
        if robot.get_left_line_sensor() == 0 and robot.get_right_line_sensor() == 0:
            ...
            robot.sleep(0.01)
            robot.set_wheels_speed(0)
        elif robot.time_from_start > 29:
            break
        elif robot.get_left_line_sensor() == 1024 and 0 in robot.get_right_line_sensors():
            robot.set_left_wheel_speed(-100)
            robot.sleep(0.01)
            robot.set_wheels_speed(0)
        elif robot.get_right_line_sensor() == 1024 and 0 in robot.get_left_line_sensors():
            robot.set_right_wheel_speed(-100)
            robot.sleep(0.01)
            robot.set_wheels_speed(0)
        else:
...
if __name__ == '__main__':
    robo = FollowerBot(start_x=169, start_y=268, track_image="track3.png")
